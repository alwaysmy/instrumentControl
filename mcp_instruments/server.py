...
def _call(model_name, connect_fn, fn):
...
def instr_discover(cidr: str | None = None) -> str:
    """发现本机所有仪器：VISA 资源列表 + LAN 端口预筛 + 多协议 *IDN?。
    ...
    return _call("discovery", lambda: None, fn)
...
def sds_status(resource: str = SDS_RES) -> str:
    ...
    return _call("SDS", lambda: SDS(resource), lambda s: s.snapshot())
...
def sds_auto_scale(ch: int, use_autoset: bool = False, resource: str = SDS_RES) -> str:
    ...
    return _call("SDS", lambda: SDS(resource),
...
def sds_screenshot(resource: str = SDS_RES) -> str:
    ...
    def fn(s: SDS):
        ...
        p = os.path.join(ROOT, "TEST_DATA", "common",
                         f"mcp_sds_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
        scope_png = s.screenshot_png(Path(p))
        return {"png": str(scope_png)}
    return _call("SDS", lambda: SDS(resource), fn)


@mcp.tool()
def sds_diagnose(resource: str = SDS_RES) -> str:
    """SDS 触发链路诊断：模式/状态/源/电平/时基（无波形时第一步）。"""
    return _call("SDS", lambda: SDS(resource), lambda s: s.diagnose_trigger())
...
def sds_shutdown(confirm: bool, resource: str = SDS_RES) -> str:
    ...
    return _call("SDS", lambda: SDS(resource), lambda s: s.shutdown(confirm=True))
...
def sdg_status(resource: str = SDG_RES) -> str:
    ...
    return _call("SDG", lambda: SDG(resource), lambda g: g.snapshot())
...
def sdg_set_wave(ch: int, wvtp: str, freq_hz: float, amp_v: float,
    ...
    return _call("SDG", lambda: SDG(resource), fn)
...
def sdg_output(ch: int, on: bool, confirm: bool = False,
    ...
    return _call("SDG", lambda: SDG(resource),
                 lambda g: (g.set_output(ch, on), time.sleep(0.3),
                            g.output_state(ch))[2])
...
def dmm_measure(function: str, resource: str = DMM_RES) -> str:
    ...
    cont/cap/diod/freq。"""
    return _call("DMM", lambda: DMM(resource), lambda d: d.measure(function))


@mcp.tool()
def dmm_status(resource: str = DMM_RES) -> str:
    """34465A 快照：IDN/选件/配置/最近读数。"""
    return _call("DMM", lambda: DMM(resource), lambda d: d.snapshot())
...
def dmm_configure(function: str, range_v: float | None = None,
    ...
    return _call("DMM", lambda: DMM(resource),
...
def dho_status(resource: str = DHO_RES) -> str:
    ...
    return _call("DHO", lambda: DHO(resource), lambda s: s.snapshot())


@mcp.tool()
def dho_measure_item(item: str, ch: int = 1, resource: str = DHO_RES) -> str:
    """DHO 单次测量查询（:MEASure:ITEM?，34 种 item 如 VPP/FREQ/VAVG）。"""
    return _call("DHO", lambda: DHO(resource), lambda s: s.measure_item(item, ch))
...
def psu_status(resource: str | None = None) -> str:
    ...
    def conn():
        res = resource or find_device("DH1766")
        return DH1766(VisaClient(res, timeout_ms=5000))
    return _call("DH1766", conn, lambda p: p.snapshot())


@mcp.tool()
def psu_measure(resource: str | None = None) -> str:
    """DH1766 三路输出电压/电流回读。"""
    def conn():
        res = resource or find_device("DH1766")
        return DH1766(VisaClient(res, timeout_ms=5000))
    return _call("DH1766", conn,
