...
- **禁止远程锁定命令**：`SYSTem:REMote ON`（SDS：禁用触摸屏/面板按键，界面显示 Remote）、
  ...
  **纯查询形式放行**（`SYST:REM?`/`SYST:COMM:RLST?`）——用于诊断面板是否被锁，
  且纯查询不改变锁定状态。**查询口判据＝单条命令单元、命令头以 `?` 结尾（问号后允许带参数）**
  ——含 `;` 的多单元消息一律拒（每次调用一条命令）。依据：SCPI 里 `;` 分隔的是**同一条消息内的
  多个命令单元**，设备会逐个执行（Keysight 手册明文；DG832 实测 `:SOUR1:PHAS?;:SOUR1:PHAS 123`
  的写单元真的生效，证据 `TEST_DATA/dg832/semicolon_units_probe_20260915.json`）。
  **写路径**（`instr_write`）仍**逐单元**查黑名单——写命令天然会用 `;` 串联。
