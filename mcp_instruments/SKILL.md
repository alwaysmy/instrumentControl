...
八位半万用表（3458A，**非 SCPI**，`ks3458a_*`）：
  ...
  **3458A 重要坑清单（写脚本/集成前逐条过；详版见 `keysight_3458a/README.md` 与
    ...
    14. **命令白名单**：只有 `keysight_3458a/commands.py` 里的命令；新增命令先登记出处
        （手册页码/实测留痕），见铁律 1。
    15. **Talk Only 模式（只讲不听）**：前面板把 `ADDRESS` 设成 **31** 会进入该模式
        （手册 p.159：`TALK` 指示灯亮、读数直接往 GPIB 送、**地址存连续内存、断电不丢**）。
        症状极具辨识度：**每条命令都"回"一个电压读数**（`ID?` 回的是 `4.99E-01` 而不是
        `HP3458A`）、`viWrite` 超时、控制器"找不到地址"。修法（前面板）：把 **ADDRESS 改成
        31 以外的值**（如 9）；或按 Reset 键（Reset 会一并回到开机测量配置）。
        `preflight --id` 会自动识别这一态并给出同样提示（退出码 5）。
