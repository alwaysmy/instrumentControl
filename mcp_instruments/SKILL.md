...
| 工具 | 用法要点 |
|---|---|
| `instr_query(resource, cmd, timeout_ms?)` | cmd 必须含 `?`；**问号后允许带参数**（`:MEASure:ITEM? VPP,CHANnel1` 是标准写法）；只读不留痕 |
...
护栏语义：查询判据＝每条 `;` 分段的**命令头带 `?`**（不是"整条以 ? 结尾"——SCPI 允许问号后带参数）；复位/存储覆写类（`*RST`/`*SAV`/`*RCL`/`:SYST:RES|FACT|PRES`，长短形式均拦）
